...
async function testLogin() {
  ...
  await assert('Login card UI elements present', async () => {
    const mtLogo = await page.$('.login-logo-mt');
    if (!mtLogo) throw new Error('MT Logo missing');
    const abcfLogo = await page.$('.login-logo-abcf');
    if (!abcfLogo) throw new Error('ABCF Logo missing');
...
async function testNavigation() {
  ...
  await assert('Songs → Settings → Setlists → Songs cycle', async () => {
    ...
    if (!text.includes('Search') && !text.includes('Songs') && !text.includes('Amazing'))
      throw new Error('Songs tab broken');
    ...
    if (!text.includes('setlist') && !text.includes('Setlist'))
      throw new Error('Setlists tab broken');

    await clickButton('Songs');
    await sleep(500);
    text = await getPageText();
    if (!text.includes('Search') && !text.includes('Songs') && !text.includes('Amazing'))
      throw new Error('Back to Songs broken');
  });
...
async function testVisualUI() {
  ...
  // --- Login page visual checks (need to sign out first) ---
  await assert('Navigate to login page for visual checks', async () => {
    await clickButton('Settings');
    await sleep(500);
    ...
    if (!signedOut) throw new Error('Could not find Sign Out button');
    await sleep(2000);
    await waitForText('Sign In', 5000);
  });

  // 1. Login white inputs
  await assert('Login inputs have white background', async () => {
    const bgColor = await page.evaluate(() => {
      const input = document.querySelector('.login-input');
      if (!input) return null;
      return window.getComputedStyle(input).backgroundColor;
    });
    if (!bgColor) throw new Error('.login-input not found');
    if (!bgColor.includes('255, 255, 255')) {
      throw new Error('Expected white background, got: ' + bgColor);
    }
  });

  // 2. Input placeholder color rule exists
  await assert('Login input placeholder has visible color', async () => {
    const ruleExists = await page.evaluate(() => {
      for (const sheet of document.styleSheets) {
        try {
          for (const rule of sheet.cssRules) {
            if (rule.selectorText && rule.selectorText.includes('login-input') && rule.selectorText.includes('placeholder')) {
              return rule.style.color || true;
            }
          }
    ...
    if (!ruleExists) throw new Error('No ::placeholder CSS rule found for .login-input');
  });

  // 3. Login card centered with max-width
  await assert('Login card is centered with max-width', async () => {
    ...
    if (!cardInfo) throw new Error('.login-card not found');
    const leftGap = cardInfo.left;
    const rightGap = cardInfo.viewportWidth - cardInfo.right;
    const diff = Math.abs(leftGap - rightGap);
    if (diff > 10) throw new Error('Card not centered. Left: ' + leftGap + ', Right: ' + rightGap);
    if (cardInfo.width > 420) throw new Error('Card too wide: ' + cardInfo.width + 'px (max 400)');
  });

  // 4. Logo rendering - both logos stacked above title
  await assert('Logos render as stacked images above title', async () => {
    const logoInfo = await page.evaluate(() => {
      const mt = document.querySelector('.login-logo-mt');
      const abcf = document.querySelector('.login-logo-abcf');
      const title = document.querySelector('.login-title');
      if (!mt || !abcf || !title) return { found: false };
      const mtRect = mt.getBoundingClientRect();
      const abcfRect = abcf.getBoundingClientRect();
      const titleRect = title.getBoundingClientRect();
      return {
        found: true,
        mtVisible: mtRect.height > 10,
        abcfVisible: abcfRect.height > 10,
        mtAboveAbcf: mtRect.bottom <= abcfRect.top + 20,
        abcfAboveTitle: abcfRect.bottom <= titleRect.top + 20,
        mtNaturalWidth: mt.naturalWidth,
        abcfNaturalWidth: abcf.naturalWidth,
      };
    });
    if (!logoInfo.found) throw new Error('Logo elements not found');
    if (!logoInfo.mtVisible) throw new Error('MT Logo not rendered (height too small)');
    if (!logoInfo.abcfVisible) throw new Error('ABCF Logo not rendered (height too small)');
    if (!logoInfo.mtAboveAbcf) throw new Error('MT Logo not stacked above ABCF Logo');
    if (!logoInfo.abcfAboveTitle) throw new Error('ABCF Logo not above title text');
    if (!logoInfo.mtNaturalWidth) throw new Error('MT Logo image failed to load');
    if (!logoInfo.abcfNaturalWidth) throw new Error('ABCF Logo image failed to load');
  });

  // 5. Login button is cyan/blue
  await assert('Sign In button has cyan background', async () => {
    ...
    if (!btnColor) throw new Error('.login-btn.primary not found');
    if (!btnColor.includes('79, 195, 247') && !btnColor.includes('79,195,247')) {
      throw new Error('Expected cyan (#4FC3F7), got: ' + btnColor);
    }
  });

  // 6. Input text color is dark on white
  await assert('Login input text color is dark (readable)', async () => {
    const textColor = await page.evaluate(() => {
      const input = document.querySelector('.login-input');
      if (!input) return null;
      return window.getComputedStyle(input).color;
    });
    if (!textColor) throw new Error('.login-input not found');
    const match = textColor.match(/(\d+),\s*(\d+),\s*(\d+)/);
    if (!match) throw new Error('Cannot parse color: ' + textColor);
    const avg = (parseInt(match[1]) + parseInt(match[2]) + parseInt(match[3])) / 3;
    if (avg > 128) throw new Error('Text too light on white bg: ' + textColor);
  });

  // Sign back in for remaining tests
  await assert('Sign back in for cleanup', async () => {
    await typeIntoInput('Email', TEST_EMAIL);
    await typeIntoInput('Password', TEST_PASSWORD);
    await clickButton('Sign In');
    await sleep(8000);
    const text = await getPageText();
    if (!text.includes('Songs') && !text.includes('Search')) {
      throw new Error('Failed to sign back in');
    }
  });

  // --- Post-login visual checks (viewer screen) ---
  await assert('Navigate to a song for visual checks', async () => {
    await clickButton('Songs');
    await sleep(500);
    ...
    if (!clicked) throw new Error('No song card to click');
    await sleep(1000);
  });

  // 7. Scrollbar via overflow-y
  await assert('Lyrics container has scrollable overflow', async () => {
    const scrollInfo = await page.evaluate(() => {
      const el = document.querySelector('.lyrics-container');
      if (!el) return null;
      const style = window.getComputedStyle(el);
  ...
  // 8. Chord color
  await assert('Chord spans use #4FC3F7 color', async () => {
    const chordColor = await page.evaluate(() => {
      // Chord spans are inside .chord-line with inline style color
      const spans = document.querySelectorAll('.chord-line span');
      for (const span of spans) {
        const c = window.getComputedStyle(span).color;
        if (c && c !== 'rgb(0, 0, 0)' && !c.includes('0, 0, 0')) return c;
      }
      return null;
    });
    if (!chordColor) throw new Error('No colored chord span found in .chord-line');
    if (!chordColor.includes('79, 195, 247') && !chordColor.includes('79,195,247')) {
      throw new Error('Expected #4FC3F7, got: ' + chordColor);
    }
  });

  // 9. Lyrics monospace font
  await assert('Lyrics use monospace font', async () => {
    const fontFamily = await page.evaluate(() => {
      const el = document.querySelector('.lyrics-container');
      if (!el) return null;
      return window.getComputedStyle(el).fontFamily;
    });
    if (!fontFamily) throw new Error('.lyrics-container not found');
    if (!fontFamily.toLowerCase().includes('courier') && !fontFamily.toLowerCase().includes('monospace')) {
      throw new Error('Expected monospace font, got: ' + fontFamily);
    }
  });

  // Return to songs
  await assert('Return to library after visual checks', async () => {
    // May be on viewer screen — try Back button first, then Songs tab
    const hasBack = await page.evaluate(() => {
      const btns = Array.from(document.querySelectorAll('button'));
      const back = btns.find(b => b.textContent.includes('Back') || b.textContent.includes('\u2190'));
    ...
    const hasSongs = await page.evaluate(() => {
      const btns = Array.from(document.querySelectorAll('button'));
      const btn = btns.find(b => b.textContent.includes('Songs'));
      if (btn) { btn.click(); return true; }
      return false;
    });
    await sleep(500);
    if (!hasSongs && !hasBack) throw new Error('Cannot navigate to songs');
  });
}
